...
export default function UniversitiesPage() {
  ...
  const getLangField = (obj: any, fieldName: string) => {
    ...
    if (fieldName === "city" || fieldName === "location") {
      return language === "ar" ? obj.city_ar || obj.city || obj.governorate : obj.city || obj.governorate || obj.location;
    }
    if (fieldName === "model") {
      return obj.educationModel || obj.model || "Egyptian";
    }
    if (fieldName === "type") {
  ...
  const getLangArray = (obj: any, fieldName: string): string[] => {
    if (!obj) return [];
    if (language === "ar" && Array.isArray(obj[fieldName + "_ar"])) return obj[fieldName + "_ar"];
    if (language === "ar" && Array.isArray(obj[fieldName + "Ar"])) return obj[fieldName + "Ar"];
    if (Array.isArray(obj[fieldName + "En"])) return obj[fieldName + "En"];
  ...
  const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>, type: "min" | "max") => {
    ...
    if (type === "min") {
      if (val > priceMax - 10000) setPriceMin(priceMax - 10000);
      else setPriceMin(val);
    } else {
      if (val < priceMin + 10000) setPriceMax(priceMin + 10000);
