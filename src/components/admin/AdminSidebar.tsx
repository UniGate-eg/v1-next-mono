...
export function AdminSidebar() {
  ...
  return (
    <aside className="w-72 flex-shrink-0 bg-[#080A11] border-r border-[#151926] flex flex-col min-h-screen p-5 justify-between select-none">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between pb-1">
          <Link href="/admin" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-medium leading-tight mt-0.5">Operations Console</p>
            </div>
          </Link>
        </div>
            ...
            .map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center justify-between px-3.5 py-2.5 rounded-2xl text-xs font-semibold transition-all duration-150 leading-relaxed ${
                    isActive
                      ? "bg-[#151929] text-white shadow-md shadow-black/50 border border-[#28314A]"
        ...
        <div className="pt-2 space-y-2.5">
          <div className="flex items-center justify-between px-2 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
            ...
            <span className="px-1.5 py-0.5 rounded-full bg-[#171B2B] text-slate-300 text-[10px] font-semibold">
          ...
          <div className="space-y-1.5">
            {pinnedInstitutions.map((inst) => (
              <div
                key={inst.code}
                className="p-2.5 rounded-2xl bg-[#0F121E] border border-[#1A2033] hover:border-[#2B3550] transition-colors flex items-center justify-between cursor-pointer group"
              >
                <div className="flex items-center gap-2.5">
                  <div className={`w-7 h-7 rounded-xl flex items-center justify-center font-bold text-[11px] border ${inst.color}`}>
                    {inst.code}
                  </div>
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 leading-tight mt-0.5">Quality Health</p>
                  </div>
                </div>
      ...
      <div className="p-4 rounded-3xl bg-gradient-to-b from-[#18152E] to-[#100D22] border border-[#352B5E] shadow-xl relative overflow-hidden space-y-2.5 mt-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-purple-400" />
            <span className="text-xs font-bold text-white">Super Admin Tier</span>
          </div>
